...
def validate(request):
	...
	if not Service_provider.objects.filter(ser_email=ser_email):
		...
		s.save()
		response['status'] = 'Registration is Sucessfull'
	else:
		response['status'] = 'Registration Failed! Email already exists'
...
def validatelogin(request):
	...
	if Service_provider.objects.filter(ser_email=ser_email,ser_psword=ser_psword):
		...
		return render_to_response('healthDirectory/regis-login.html', {'title':' Email and Password does not match!'},context_instance=RequestContext(request))
